package com.esp;
  ...
  public class EspScreen extends Screen {
      ...
      private static final int PW = 380, PH = 248;
      private int px, py;
      ...
      protected void init() {
          ...
          addRenderableWidget(tabBtn("\u25B6 Ore X-Ray",  1, px+3+tabW,    py+22, tabW));
          addRenderableWidget(tabBtn("\u25B6 Movement",   2, px+4+tabW*2,  py+22, tabW-1));
      ...
      private Button tabBtn(String label, int id, int x, int y, int w) {
          String txt = TAB == id ? "\u00A7e\u00A7l" + label.substring(2) : "\u00A77" + label.substring(2);
      ...
      private void buildEsp(int x, int y, int w) {
          ...
          // Color sliders R/G/B — leave 26px on right for preview box
          int slW = (w - 6 - 26) / 3;
      ...
      private void buildOre(int x, int y, int w) {
          ...
          addRenderableWidget(new AbstractSliderButton(x, y, w, 18,
                  ...
                  (EspConfig.oreRange - 8.0) / (128.0 - 8.0)) {
              @Override protected void updateMessage() { setMessage(Component.literal("Scan: "+(int)(8+value*(128-8))+" blocks")); }
              @Override protected void applyValue()    { EspConfig.oreRange = (int)(8+value*(128-8)); EspConfig.save(); }
          });
      }

      // ── Movement tab ────────────────────────────────────────────────────────
      private void buildMov(int x, int y, int w) {
          addRenderableWidget(CycleButton.<Boolean>onOffBuilder(EspConfig.flyEnabled)
              .create(x, y, (w-4)/2, 18, Component.literal("Fly [F]"),
                  (b, v) -> { EspConfig.flyEnabled = v; EspConfig.save(); }));
          y += 22;

          double initSpd = Math.min(EspConfig.flySpeed / 4.0, 1.0);
          addRenderableWidget(new AbstractSliderButton(x, y, w, 18,
                  Component.literal("Fly Speed: " + Math.round(EspConfig.flySpeed * 100) + "%"), initSpd) {
              @Override protected void updateMessage() { setMessage(Component.literal("Fly Speed: "+(int)(value*400)+"%")); }
              @Override protected void applyValue()    { EspConfig.flySpeed = (float)(value * 4.0); EspConfig.save(); }
          });
      }
      ...
      public void render(GuiGraphics g, int mx, int my, float pt) {
          ...
          // Header gradient (dark blue → dark purple)
          ...
          // Tab indicator line under active tab
          int tabW = (PW - 6) / 3;
          ...
          if (TAB == 0) {
              ...
              int bpx = px + PW - 36;
              int bpy = py + 46 + 22 * 3;
              // Checkerboard hint (so transparent black is visible)
          ...
          // ── Movement info ────────────────────────────────────────────────────
          if (TAB == 2) {
              String flyState = EspConfig.flyEnabled ? "\u00A7aON" : "\u00A7cOFF";
              g.drawString(font, "Fly: " + flyState, px+10, py+46+22*2+8, 0xFFFFFF);
              g.drawString(font, "\u00A77Speed: " + Math.round(EspConfig.flySpeed*100) + "%", px+10, py+46+22*2+20, 0xAAAAAA);
              g.drawString(font, "\u00A78Works on servers without anti-cheat", px+10, py+46+22*2+32, 0x666666);
          }

          // Keys hint at bottom
          g.drawCenteredString(font, "\u00A78[G] Toggle  [H] Menu  [J] Ore  [F] Fly", px+PW/2, py+PH-12, 0x555555);
